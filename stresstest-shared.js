...
class StresstestShared {
  ...
  async start() {
    ...
    let maxTxChain = 24
    let feePerTx = BITBOX.BitcoinCash.getByteCount({ P2PKH: 1 }, { P2PKH: 1 }) + 9
    ...
    while (satsChange < dustLimitSats) {
      ...
      byteCount = BITBOX.BitcoinCash.getByteCount({ P2PKH: 1 }, { P2PKH: numAddresses + 1 }) + 9
  ...
  createTx(wallet, targetAddress) {
    ...
    let byteCount = BITBOX.BitcoinCash.getByteCount({ P2PKH: 1 }, { P2PKH: 1 }) + 9
